...
def scrape_endpoint(body: Dict[str, Any]):
    ...
    # 2) Parse static HTML safely
    try:
        # prefer lxml, fallback handled in scraper if needed
        ...
        result["meta"].update({"title": meta_title, "description": description, "language": lang or "", "canonical": canonical})
    ...
    # 3) JS fallback heuristic (if content too small or no sections)
    total_text_len = sum(len(s.get("content", {}).get("text", "") or "") for s in result["sections"])
    if total_text_len < 300 or len(result["sections"]) == 0:
        # Prefer the strong flow for known anti-bot targets or if we got 403 earlier.
        try:
            # First attempt the normal JS flow
            js_result = js_scrape_with_playwright(url, max_scrolls=3)
            # If that returned errors suggesting blocking, try the hard flow
            if js_result.get("errors") and any("403" in e.get("message", "") or "blocked" in e.get("message", "").lower() for e in js_result.get("errors", [])):
                LOG.info("js_scrape_with_playwright returned blocking-like errors, trying js_scrape_hard")
                js_result = js_scrape_hard(url, max_scrolls=8, headless=False)
        except Exception as e:
            LOG.exception("Playwright normal flow failed")
            js_result = {"sections": [], "meta": {}, "interactions": {"clicks": [], "scrolls": 0, "pages": []}, "errors": [{"message": str(e), "phase": "render"}]}

        # Merge results safely
        try:
            if js_result.get("meta"):
                for k, v in js_result["meta"].items():
                    if v and not result["meta"].get(k):
                        result["meta"][k] = v

            existing_raw = {s.get("rawHtml")[:200] for s in result["sections"] if s.get("rawHtml")}
            for sec in js_result.get("sections", []):
                if (sec.get("rawHtml") or "")[:200] not in existing_raw:
...
if os.path.isdir(frontend_dist):
    ...
    def root_info():
        return {"message": "Frontend not built. Run the frontend build (npm run build). API: /healthz, /scrape"}
